...
async def test_continue_shopping(log_as_standard_user):
    ...
    page = await get_first_page(log_as_standard_user)
    inventory_page = InventoryPage(page)
    inventory_page = await inventory_page.click_generic_item(0)
    inventory_page = await inventory_page.click_card()
    inventory_page = inventory_page.navigate_to(Pages.CardPage)
    inventory_page = await inventory_page.verify_card_quantity('1')
    inventory_page = await inventory_page.click_continue_shopping()
